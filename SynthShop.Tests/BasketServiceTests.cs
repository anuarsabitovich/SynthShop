    ...
    public class BasketServiceTests
    {
        ...
        public async Task AddItemToBasketAsync_ShouldAddNewItem_WhenItemDoesNotExistInBasket()
        {
            ...
            await _basketItemRepository.Received(1).CreateBasketItemAsync(
                Arg.Is<BasketItem>(item => item.BasketId == basketId && item.ProductId == productId && item.Quantity == quantity));
        }
        ...
        public async Task AddItemToBasketAsync_ShouldUpdateQuantity_WhenItemExistInBasket()
        {
            ...
            var basket = new Basket
            {
                ...
                {
                    new BasketItem { BasketItemId = Guid.NewGuid(), BasketId = basketId, ProductId = productId, Quantity = initialQuantity }
                }
            };
            ...
            await _basketItemRepository.Received(1).UpdateBasketItemAsync(updatedItem.BasketItemId, Arg.Is<BasketItem>(item => item.Quantity == initialQuantity + additionalQuantity));
        }
        ...
        public async Task DeleteItemFromBasketAsync_ShouldRemoveItem_WhenItemExists()
        {
            ...
            var basket = new Basket
            {
                ...
                Items = new List<BasketItem>
                {
                    new BasketItem { BasketItemId = basketItemId, BasketId = basketId, ProductId = Guid.NewGuid(), Quantity = 1 }
                }
            };
        ...
        public async Task DeleteItemFromBasketAsync_ShouldLogWarning_WhenItemDoesNotExist()
        {
            ...
            _logger.Received(1).Warning("Item with ID {BasketItemId} not found in basket {BasketId}", basketItemId, basketId);
        }
        ...
        public async Task UpdateItemInBasket_ShouldUpdateItem_WhenItemExists()
        {
            ...
            // Assert
            await _basketItemRepository.Received(1).UpdateBasketItemAsync(basketItemId, Arg.Is<BasketItem>(item => item.Quantity == newQuantity));
        }
