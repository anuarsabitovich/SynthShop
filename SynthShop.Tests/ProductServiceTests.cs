...
namespace SynthShop.Tests
{
    public class ProductServiceTests
    {
        ...
        {
            // Arrange
            var products = new PagedList<Product>(new List<Product> { new Product { Name = "Product1" } }, 1, 1, 10);
            _productRepository.GetAllAsync(Arg.Any<Expression<Func<Product, bool>>>(), Arg.Any<string>(), Arg.Any<bool>(), Arg.Any<int>(), Arg.Any<int>()).Returns(Task.FromResult(products));
        ...
        public async Task UpdateAsync_ShouldUpdateProduct_WhenProductExists()
        {
            ...
            var updatedProduct = new Product { Name = "UpdatedProduct", Description = "UpdatedDescription", Price = 100, StockQuantity = 50, CategoryID = Guid.NewGuid() };
