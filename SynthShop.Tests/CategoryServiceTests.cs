...
namespace SynthShop.Tests
{
    public class CategoryServiceTests
    {
        ...
        public async Task CreateAsync_ShouldCreateCategory_WhenCategoryNameIsUnique()
        {
            ...
            _categoryRepository.GetAllAsync(Arg.Any<Expression<Func<Category, bool>>>()).Returns(Task.FromResult(existingCategories));
        ...
        {
            // Arrange
            var categories = new PagedList<Category>(new List<Category> { new Category { Name = "Category1" } }, 1, 1, 10);
            _categoryRepository.GetAllAsync(Arg.Any<Expression<Func<Category, bool>>>(), Arg.Any<string>(), Arg.Any<bool>(), Arg.Any<int>(), Arg.Any<int>()).Returns(Task.FromResult(categories));
