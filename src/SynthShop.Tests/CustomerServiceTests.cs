...
public class CustomerServiceTests
{
    ...
    public async Task GetAllAsync_ShouldReturnPagedList_WhenCalled()
    {
        ...
        var users = new List<User>
        {
            ...
            new() { Id = Guid.NewGuid(), FirstName = "Jane", LastName = "Doe", Email = "jane.doe@example.com" }
        };
        var pagedList = new PagedList<User>(users, users.Count, pageNumber, pageSize);

        _customerRepository
            .GetAllAsync(Arg.Any<Expression<Func<User, bool>>>(), sortBy, isAscending, pageNumber, pageSize)
