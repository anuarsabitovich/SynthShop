...
public class CustomerService : ICustomerService
{
    ...
    public CustomerService(ICustomerRepository customerRepository, ILogger logger,
        IOptions<PagingSettings> pagingSettings, IUnitOfWork unitOfWork)
    {
    ...
    {
        Expression<Func<User, bool>> filter = searchTerm is not null
            ? x => x.FirstName.Contains(searchTerm) || x.LastName.Contains(searchTerm) ||
                   x.UserName.Contains(searchTerm)
            : null;
        return await _customerRepository.GetAllAsync(filter, sortBy, IsAscending ?? true, pageNumber,
            pageSize ?? _pagingSettings.PageSize);
    }
