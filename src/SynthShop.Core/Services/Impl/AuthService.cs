...
public class AuthService : IAuthService
{
    ...
    private bool ValidateAccessToken(ClaimsPrincipal validatedToken)
    {
        ...
        if (expiryDateTimeUtc > DateTime.UtcNow) return false;
    ...
    private bool ValidateRefreshToken(RefreshToken storedRefreshToken, string jti)
    {
        ...
        if (DateTime.UtcNow > storedRefreshToken.ExpiryDate) return false;

        if (storedRefreshToken.Invalidated) return false;

        if (storedRefreshToken.Used) return false;
